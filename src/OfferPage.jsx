  ...
  const payment = [
    {
      ...
      "discount" : "Assured cashback between ₹25 to ₹100 on Paytm wallet transactions above ₹399"
    },
    {
      ...
      <TabContext value={value}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <TabList onChange={handleChange} aria-label="lab API tabs example">
            <Tab label="Restaurant offers" value="1" sx={{fontSize : "13px" ,  margin : "10px"}}/>
            <Tab label="Payment offers/Coupons" value="2" sx={{fontSize : "13px" ,  margin : "10px"}} />
          </TabList>
        </Box>
        <TabPanel value="1"><ProductCard/></TabPanel>
        <TabPanel className='paym' value="2">{payment.map((data,index)=> <PaymentOffers bank={data} key={index}/>)}</TabPanel>
      </TabContext>
    </Box>
  );
}
  ...
  return(
    <div className="payment-offers">
        <div className="payment-one">
            ...
            <span><strong>{bank.code}</strong></span>
        </div>
        ...
        <div className="payment-four">
          <Button sx={{borderRadius : 0}} variant='outlined'><strong>COPY CODE</strong></Button>
        </div>
    </div>
  )
}
